...
xq = 1344*sin(0.06*pi*n)+864*sin(0.18*pi*n)+8543*sin(0.38*pi*n)-43*sin(0.8*pi*n);
    data=xi+j*xq;
    frame(k,:)=data;
        power =sum(abs(data.^2))/length(data);
    signal_power_dBW =10*log10(power);
end
% str = importdata("C:\Users\DELL\Input_GroupID_1A_FreqID_60_SNR_10.000000_Foff_0.000000_Nframe_1_Nstart_0.txt");
% 
% for i =1:1024
...
%     signal_power_dBW =10*log10(power);
% end
noise_power_dBW=signal_power_dBW -SNR;
